...
{
    return static_cast<ARAIPCCFMessageEncoder*> (messageEncoder)->copyDictionary ();
}
...
__attribute__((cf_returns_retained)) CFDictionaryRef ARAIPCCFCopyMessageEncoderDictionaryAddingMessageID (ARAIPCMessageEncoder* messageEncoder, ARAIPCMessageID messageID)
{
    ...
    const auto dictionary { static_cast<ARAIPCCFMessageEncoder*> (messageEncoder)->copyDictionary () };
...
{
    return static_cast<ARAIPCCFMessageEncoder*> (messageEncoder)->createMessageEncoderData ();
}
...
ARAIPCMessageID ARAIPCCFGetMessageIDFromDictionary (ARAIPCMessageDecoder* messageDecoder)
{
    ...
    bool success { static_cast<ARAIPCCFMessageDecoder*>(messageDecoder)->readInt32 (ARAIPCCFMessageGetEncodedKey (messageIDKey, true), &result) };
