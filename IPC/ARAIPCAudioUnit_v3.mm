...
public:
                      ...
                      ARAIPCReplyHandler * replyHandler, void * replyHandlerUserData) override
    {
        @autoreleasepool
        {
            ...
            do
            {
                ...
                if (!receivedMessage)
                    continue;
                _handleReceivedMessage ((__bridge CFDictionaryRef) receivedMessage->_message, receivedMessage->_userData);
    ...
    {
        if (_sendingThread.load (std::memory_order_acquire) != nil)
        {
            ...
            if (_dispatchRemoteTransactions)
            {
                ...
                dispatch_async (dispatch_get_main_queue (),
                    ^{
                        ...
                        _handleReceivedMessage ((__bridge CFDictionaryRef) message, userData);
                    });
            }
            else
            {
...
private:
    ...
    void _handleReceivedMessage (CFDictionaryRef message, void * userData)
    {
        const auto currentThread { [NSThread currentThread] };
        ...
        if (messageID != 0)
        {
            ARA_IPC_LOG ("received message with ID %i%s", messageID, (_awaitsReply) ? " (while awaiting reply)" : "");
...
#else
    std::atomic<NSThread * __unsafe_unretained> _sendingThread {};  // if set, the receive callback forward to that thread
#endif
    bool _awaitsReply { false };
...
protected:
    ...
    {
        if (auto callHostBlock { getMessageChannel ().callHostBlock })
        {
...
{
#if __has_feature(objc_arc)
    return _sharedPlugInCallbacksSender->processReceivedMessage (message, (__bridge void *)audioUnit);
#else
    return _sharedPlugInCallbacksSender->processReceivedMessage (message, audioUnit);
#endif
}
